...
## 11. EXP-008: MLP 강화 결과 (epochs 50→80, patience 15→20)

### 변경 사항
- `MAX_EPOCHS`: 50 → **80**
- `PATIENCE`: 15 → **20**
- `T_max` (CosineAnnealingLR): 50 → **80** (cycle 일치)
- 그 외 동일 (lr=1e-3, width 256-128-64, dropout 0.2, single seed)

### Fold별 결과
| Fold | best AUC | best_epoch | epochs_used |
|---|---|---|---|
| 0 | 0.94251 | 79 | 80 (max) |
| 1 | 0.93995 | 70 | 80 (max) |
| 2 | 0.94051 | 65 | 80 (max) |
...
- **모든 fold가 80 epoch까지 감** — best_epoch는 65~79 다양. patience=20 안 걸림 (80 도달이 한계)
- **Blend opt OOF: 0.95053 = 0.95053 (동점)** — MLP 강화가 blend 안에서 흡수됨
- **opt 가중치 변화**: `(0.25, 0.25, 0.45, 0.05)` → **`(0.25, 0.25, 0.40, 0.10)`**. CAT 비중 줄고 MLP 비중 2배
- **MLP 한계 효과**: +0.00006 → **+0.00011** (4-way − 3-way opt)
- **소요시간**: MLP 12.5분 → 20.6분 (1.6배, epoch 1.6배 비례)

### 인사이트
- 학습 미수렴 가설 옳음 — epoch 늘리면 단독은 확실히 오름
- 그러나 blend OOF 천장 0.95053은 안 깨짐 — 단일 강화가 weight redistribution으로 상쇄
- **opt에서 w_mlp 2배 증가가 진짜 다양성 시그널** — 같은 OOF여도 LB는 다를 수 있음
- 모든 fold가 max에 닿았으므로 추가 epoch (120 등) 또는 lr scheduler 변경 (warm restart) 시도 가치 있음

### LB 결과 (확정)
- **`submission_exp008_blend_opt.csv` LB: 0.94973** (vs EXP-007 0.94979, **-0.00006**)
- OOF 동점인데 LB 하락 → MLP 강화의 가중치 변화가 **OOF-친화적**
- CV-LB 격차 0.00074 → **0.00080** 확대 — overfitting 방향
- **결론**: MLP solo OOF 강화가 LB로 전이 안 됨. **EXP-007 (LB 0.94979)이 자체 학습 천장**. 추가 epoch/multi-seed/width↑는 noise 영역. 격차 좁히려면 외부 RealMLP 같은 자원 필요.
