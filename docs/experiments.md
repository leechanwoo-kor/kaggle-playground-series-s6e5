...
| **EXP-004 lr=0.02 blend** | **0.95042** | — | — | 단일 모두 +0.0003 — 진단(천장 ≈ 0.94915) 부정확함 확인 |
...
- **블렌딩 LB 전이율**: EXP-002 OOF Δ+0.00104 → LB Δ+0.00068 (65%); EXP-003 OOF Δ+0.00020 → LB Δ+0.00022 (1:1). 단일 모델 천장 풀어주는 변화는 LB로 거의 그대로 전이, 블렌딩 추가만으로 얻는 OOF는 LB로 일부만 전이됨. → 다음 OOF 짜기는 단일 모델 개선 우선.
- **CV-LB 격차**: 0.00032 (single) → 0.00068 (blend). 격차 안정. 블렌딩이 OOF를 살짝 친화적으로 짜는 경향 — OOF만 짜는 시도 위험.
...
- [x] EXP-004: lr=0.02 미세 튜닝 — Blend OOF **0.95042** (단일 모두 +0.0003)
- [ ] EXP-004 LB 점수 기록 (제출 후)
- [ ] EXP-005 후보: FE (Driver 빈도/타깃 인코딩), 또는 lr=0.01 추가 짜기, 마무리에 multi-seed
