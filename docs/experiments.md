...
| 단계 | LB | Δ |
|---|---|---|
| EXP-001 baseline (XGB) | 0.94874 | — |
| EXP-002 3-way blend | 0.94942 | +0.00068 |
| EXP-003 GPU + CAT iter↑ | 0.94964 | +0.00022 |
| EXP-004 lr=0.02 | 0.94977 | +0.00013 |
| EXP-007 +기본 MLP | 0.94979 | +0.00002 |
| **EXP-010 +PLR-MLP** ⭐ | **0.94987** | **+0.00008** |

**누적 향상: +0.00113** (EXP-001 → EXP-010, 자체 구현만으로)

가장 큰 단일 도약: **EXP-001 → EXP-002 blend** (+0.00068, 모델 다양성), 두 번째: **EXP-009/010 PLR-MLP** (+0.00008, 알고리즘 family 추가). 그 외 미세 조정(multi-seed, MLP 강화, inner ens, FE)은 모두 ±0.00006 영역.
...
- **EXP-010이 자체 학습 LB 천장 (LB 0.94987)**: EXP-013 LB 0.94986 (-0.00001). OOF 미세 향상이 LB로 안 옴. **알고리즘 family 추가 (PLR)** 가 진짜 LB 도약, 그 위 보조 기법(multi-seed, MLP 강화, inner ens)은 모두 한계 효용. 외부 자원 없이 자체 구현으로 EXP-001 baseline (LB 0.94874) → EXP-010 (LB 0.94987) **+0.00113 향상**.
...
- [x] EXP-013 LB — opt **0.94986** (-0.00001 vs EXP-010). OOF +0.00002 → LB -0.00001 (전이 X)
- **컴페 마무리 — 자체 학습 LB best: EXP-010 0.94987**
